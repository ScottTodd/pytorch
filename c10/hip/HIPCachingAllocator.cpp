...
// HIP graphs helper
struct PrivatePool {
  PrivatePool(MempoolId_t id, HIPAllocator* allocator = nullptr)
      : id(std::move(id)),
...
class DeviceCachingAllocator {
 ...
 public:
  ...
  DeviceCachingAllocator()
    ...
    stats.max_split_size =
        static_cast<int64_t>(HIPAllocatorConfig::max_split_size());
  ...
  // Insert "free marker" (empty nodes) into the HIP graph for all streams that
  ...
  std::vector<hipGraphNode_t> insert_free_marker(Block* block) {
    std::vector<hipGraphNode_t> empty_nodes;

    auto try_add_empty_node = [&](hipStream_t stream) -> bool {
      ...
      TORCH_INTERNAL_ASSERT(
          status != hipStreamCaptureStatusInvalidated,
      ...
      hipGraphNode_t node{};
      C10_HIP_CHECK(hipGraphAddEmptyNode(&node, graph, deps, num_deps));
#if (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 13000)
      C10_HIP_CHECK(hipStreamUpdateCaptureDependencies(
          stream, &node, nullptr, 1, hipStreamSetCaptureDependencies));
#else
      C10_HIP_CHECK(hipStreamUpdateCaptureDependencies(
  ...
  // Returns the current set of "terminal" nodes in the HIP graph for a given
  ...
  std::vector<hipGraphNode_t> get_terminals(hipStream_t stream) {
    std::vector<hipGraphNode_t> result;
  ...
  ska::flat_hash_set<hipGraphNode_t> get_reusable_empty_nodes(
      hipStream_t stream) {
    ...
    auto get_dependencies = [](hipGraphNode_t node,
                               hipGraphNode_t* pDependencies,
                               size_t* pNumDependencies) -> void {
#if (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 13000)
      C10_HIP_CHECK(hipGraphNodeGetDependencies(
    ...
    auto is_empty_node = [](hipGraphNode_t n) -> bool {
      hipGraphNodeType type{};
      C10_HIP_CHECK(hipGraphNodeGetType(n, &type));
      return type == hipGraphNodeTypeEmpty;
    };
    ...
    // terminal nodes.
    ska::flat_hash_map<hipGraphNode_t, size_t> num_terminals_reachable;

    for (auto terminal : terminals) {
      ska::flat_hash_set<hipGraphNode_t> visited;
      ska::flat_hash_set<hipGraphNode_t> empty_nodes;

      std::function<void(hipGraphNode_t)> reverse_dfs =
          [&](hipGraphNode_t node) {
    ...
    ska::flat_hash_set<hipGraphNode_t> reusable_empty_nodes;
  ...
  ShareableHandle shareIpcHandle(Block* block) {
    ...
    if (!block->expandable_segment_) {
      ...
      ss.write((char*)&handle, HIP_IPC_HANDLE_SIZE);
    } else {
 ...
 private:
  ...
  bool get_free_block(AllocParams& p) {
    ...
    if ((p.size() >= HIPAllocatorConfig::max_split_size()) &&
        ((*it)->size >=
         p.size() + HIPAllocatorConfig::max_non_split_rounding_size()))
      return false;
    p.block = *it;
 ...
 public:
  ...
  SnapshotInfo snapshot(MempoolId_t mempool_id) override {
    ...
    md.garbage_collection_threshold =
        HIPAllocatorConfig::garbage_collection_threshold();
    md.max_split_size = HIPAllocatorConfig::max_split_size();
    ...
    md.last_allocator_settings = HIPAllocatorConfig::last_allocator_settings();
    md.graph_capture_record_stream_reuse =
        HIPAllocatorConfig::graph_capture_record_stream_reuse();
    md.roundup_power2_divisions =
        HIPAllocatorConfig::roundup_power2_divisions();
  ...
  struct MemHandleCacheEntry {
    MemHandleCacheEntry(
      ...
      if (type == SHAREABLE_HIP_MALLOC) {
        ...
        ss.read((char*)&hip_handle, HIP_IPC_HANDLE_SIZE);
...
// order (SIOF) issues.
#define HIP_MASQUERADING_AS_HIP \
  "cud"                          \
  "a"
...
//
// uuid_ is incremented when HIPGraph creates a MemPool
// as a result of a user not providing a pool.
//
// MempoolId_t of {0, 0} is used to denote when no MemPool has been
// passed to a function, either by user or HIPGraphs. For example,
...
MemPool::MemPool(
  ...
  HIPCachingAllocator::createOrIncrefPool(device_, id_, allocator);
  if (use_on_oom) {
    HIPCachingAllocator::setUseOnOOM(device_, id_);
  }
}

MemPool::~MemPool() {
  TORCH_INTERNAL_ASSERT(use_count() == 1);
  HIPCachingAllocator::releasePool(device_, id_);
  c10::hip::HIPCachingAllocator::emptyCache(id_);
}
...
int MemPool::use_count() {
  return HIPCachingAllocator::getPoolUseCount(device_, id_);
}
