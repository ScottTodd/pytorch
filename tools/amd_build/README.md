...
2. Prepare source files (optional, build scripts do this without committing)

    ```bash
    # 1. HIPIFY the codebase and commit the changes to keep history clean
    python tools/amd_build/build_amd.py
    git checkout -b amd-build
    git add -A
    git commit -m "DO NOT SUBMIT - HIPIFY"

    # 2. Write _rocm_init.py (this is ignored by .gitignore, fine to overwrite)
